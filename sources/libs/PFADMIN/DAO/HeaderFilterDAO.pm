...
# https://akscf.me/
# ******************************************************************************************
package PFADMIN::DAO::HeaderFilterDAO;
...
sub new ($$;$) {
	...
	my $self = {
		...
		pfadmin         	=> $pfadmin,
		wstk				=> $pfadmin->{wstk},
		filter_file			=> $pfadmin->get_config('postfix','header_filter'),
		filter_enabled 		=> 1
	};
	bless( $self, $class );
    #
...
sub lock {
    ...
    if($action == 1) {
        my $v = $wstk->sdb_get('lock_header_filter');
        if($v) { die Wstk::WstkException->new( 'Resource is locked, try again later', RPC_ERR_CODE_INTERNAL_ERROR ); }
        $wstk->sdb_put('lock_header_filter', 1);
    } else {
        $wstk->sdb_put('lock_header_filter', undef);
    }
}
