...
# https://akscf.me/
# ******************************************************************************************
package PFADMIN::DAO::SenderFilterDAO;
...
sub new ($$;$) {
	...
	my $self = {
		...
		pfadmin         	=> $pfadmin,
		wstk				=> $pfadmin->{wstk},
		filter_file			=> $pfadmin->get_config('postfix','sender_filter'),
		filter_enabled 		=> 1,
		backup_path			=> undef,
		db_path				=> undef,
		db_file				=> undef,
	...
	$self->{db_file} = $self->{db_path}.'/sender_filter.db';
	$self->{id_file} = $self->{db_path}.'/sender_filter.id';
	#
