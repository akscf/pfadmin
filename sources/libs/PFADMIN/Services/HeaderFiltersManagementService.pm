...
# ******************************************************************************************
package PFADMIN::Services::HeaderFiltersManagementService;
...
sub rpc_readBody {
	...
	#
	return $self->{header_filter_dao}->read();
}
...
sub rpc_writeBody {
	...
	check_permissions($self, $sec_ctx, [ROLE_ADMIN]);
	#
	return ($self->{header_filter_dao}->write($data) ? Wstk::Boolean::TRUE : Wstk::Boolean::FALSE);
}
